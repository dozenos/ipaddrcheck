...
C utility for IPv4/IPv6 address and prefix validation in shell scripts. Used by DozenOS as the canonical "is this string a valid IP / network / host / CIDR?" checker invoked from XML validators and conf-mode scripts. Fast and exit-status-driven (intended for `if ipaddrcheck --is-ipv4...; then...`).
...
One of the canonical 14 DozenOS-image build packages, listed in an internal repository. Built into DozenOS images by `dozenos-build` and invoked at runtime by `dozenos-1x` validators and conf-mode scripts. Sits in the §3.5 native-libraries category alongside `hvinfo`, `udp-broadcast-relay`, etc.

## Conventions

- Commit / PR title format: `component: T12345: description` (Phorge task ID at https://dozenos.dev mandatory). Enforced by `dozenos/.github` reusable workflows where consumed.
- Branch model: `rolling`, `circinus` (1.5 LTS), `sagitta` (1.4 LTS), `equuleus` (1.3 LTS).
- Maintained by the DozenOS team (`maintainers@dozenos.local`); license is GPL-2 + LGPL-2.1 dual.
